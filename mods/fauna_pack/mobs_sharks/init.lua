...
-- local variables
	...
	local l_spawn_in		= {"element:water_source"}
	local l_spawn_near		= {"element:water_source","seawrecks:woodship","seawrecks:uboot"}
	local l_spawn_chance	= 120000

-- load settings
	dofile(minetest.get_modpath("mobs_sharks").."/SETTINGS.txt")
	if not mobs.ENABLE_SHARK_LARGE then
		l_spawn_chance = l_spawn_chance - 40000
	end
	if not mobs.ENABLE_SHARK_MEDIUM then
		l_spawn_chance = l_spawn_chance - 40000
	end
	if not mobs.ENABLE_SHARK_SMALL then
		l_spawn_chance = l_spawn_chance - 40000
	end

-- large
	if mobs.ENABLE_SHARK_LARGE then
		mobs:register_mob("mobs_sharks:shark_lg", {
			...
			do_custom = function(self)
				if mobs.HELP_WITH_EXPERIMENT then
		...
		mobs:spawn_specific("mobs_sharks:shark_lg", l_spawn_in, l_spawn_near, -1, 20, 30, l_spawn_chance, 1, -255, -24)
		mobs:register_egg("mobs_sharks:shark_lg", "Shark (large)", l_egg_texture, 0)
	end

-- medium
	if mobs.ENABLE_SHARK_MEDIUM then
		...
		--name, nodes, neighbours, minlight, maxlight, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs_sharks:shark_md", l_spawn_in, l_spawn_near, -1, 20, 30, l_spawn_chance, 1, -255, -12)
		mobs:register_egg("mobs_sharks:shark_md", "Shark (medium)", l_egg_texture, 0)
	end

-- small
	if mobs.ENABLE_SHARK_SMALL then
		...
		--name, nodes, neighbours, minlight, maxlight, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs_sharks:shark_sm", l_spawn_in, l_spawn_near, -1, 20, 30, l_spawn_chance, 1, -255, -6)
