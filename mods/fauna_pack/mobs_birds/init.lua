...
-- local variables
	...
	local l_spawn_near_gull		= {"element:water_source"}
	local l_spawn_near_bird		= {"group:leaves", "tree:pine_leaves", "tree:jungle_leaves", "plant:cactus"}
	local l_spawn_chance_gull	= 72000
	local l_spawn_chance_bird	= 36000

-- load settings
	dofile(minetest.get_modpath("mobs_birds").."/SETTINGS.txt")
	if not mobs.ENABLE_LARGE_BIRDS then
		l_spawn_chance_bird = l_spawn_chance_bird - 18000
	end
	if not mobs.ENABLE_SMALL_BIRDS then
		l_spawn_chance_bird = l_spawn_chance_bird - 18000
	end

-- gulls
	if mobs.ENABLE_GULLS then
		...
		mobs:spawn_specific("mobs_birds:gull", l_spawn_in, l_spawn_near_gull, 5, 20, 30, l_spawn_chance_gull, 1, 10, 1023)
		mobs:register_egg("mobs_birds:gull", "Gull", l_egg_texture, 1)
	end

-- large birds
	if mobs.ENABLE_LARGE_BIRDS then
		...
		--name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs_birds:bird_lg", l_spawn_in, l_spawn_near_bird, 5, 20, 30, l_spawn_chance_bird, 1, 10, 2046)
		mobs:register_egg("mobs_birds:bird_lg", "Large bird", l_egg_texture, 1)
	end

-- small birds
	if mobs.ENABLE_SMALL_BIRDS then
		...
		--name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height
		mobs:spawn_specific("mobs_birds:bird_sm", l_spawn_in, l_spawn_near_bird, 5, 20, 30, l_spawn_chance_bird, 1, 10, 1023)
