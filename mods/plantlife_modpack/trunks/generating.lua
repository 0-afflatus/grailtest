...
-- 	big twigs
	if abstract_trunks.Big_Twigs == true then
--	big twig 1		
...
if abstract_trunks.Twigs_on_ground == true then
biome_lib:register_generate_plant({
    surface = {"default:dirt_with_grass", "default:dirt_with_ice"},
    max_count = abstract_trunks.Twigs_on_ground_Max_Count,
    rarity = abstract_trunks.Twigs_on_ground_Rarity,
    min_elevation = 1,
	max_elevation = 80,
...
if abstract_trunks.Twigs_on_water == true then
biome_lib:register_generate_plant({
    surface = {"group:water"},
    max_count = abstract_trunks.Twigs_on_water_Max_Count,
    rarity = abstract_trunks.Twigs_on_water_Rarity,
    min_elevation = 1,
	max_elevation = 60,
...
--	  MoD 						 TRuNK				NR
	...
	{"moretrees",				"pine_trunk",			 	11},
	{"moretrees",				"spruce_trunk",				16},
	{"moretrees",				"willow_trunk",				17},
}

if abstract_trunks.Horizontal_Trunks == true then -- see settings.txt
for i in pairs(TRuNKS) do
	...
	and minetest.registered_nodes[trunkname] then -- the node being called exists.
			local temptrunk = clone_node(trunkname)
...
abstract_trunks.place_trunk = function(pos)
	...
	local near_here = minetest.find_node_near(right_here, 7, {"group:tree", "group:sapling"})
	local near_node = nil
	if near_here ~= nil then
		near_node = minetest.get_node(near_here)
	else
		near_node = node_here
	end
	...
	if minetest.registered_nodes[node_here.name].buildable_to then -- instead of check_air = true,
		for i in pairs(TRuNKS) do
			...
			local 	NR = 			TRuNKS[i][3]
			local	trunkname = MoD..":"..TRuNK
			local	sapname = string.gsub(trunkname, "trunk", "sapling_ongen")
			
			if near_node.name == sapname or near_node.name == trunkname or chance == NR then
				...
				elseif trunk_type == 2 and abstract_trunks.Horizontal_Trunks == true then
					if minetest.get_modpath(MoD) ~= nil then
						if minetest.registered_nodes[node_north.name].buildable_to then
				...
				elseif trunk_type == 3 and abstract_trunks.Horizontal_Trunks == true then
					if minetest.get_modpath(MoD) ~= nil then
						if minetest.registered_nodes[node_west.name].buildable_to then
...
biome_lib:register_generate_plant({
	surface = {"default:dirt_with_grass", "default:dirt_with_ice"},
	max_count = abstract_trunks.Trunks_Max_Count, -- 320,
	rarity = abstract_trunks.Trunks_Rarity, -- 99,
	min_elevation = 1,
	max_elevation = 80,
...
-----------------------------------------------------------------------------------------------
if abstract_trunks.Moss_on_ground == true then
...
biome_lib:register_generate_plant({
	surface = {"default:dirt_with_grass", "default:dirt_with_ice"},
	max_count = abstract_trunks.Moss_on_ground_Max_Count,
	rarity = abstract_trunks.Moss_on_ground_Rarity,
	min_elevation = 1,
	max_elevation = 40,
	near_nodes = {
		"group:tree",
...
-----------------------------------------------------------------------------------------------
if abstract_trunks.Moss_on_trunk == true then
abstract_trunks.grow_moss_on_trunk = function(pos)
	...
	--if minetest.get_item_group(node_under.name, "tree") < 1 then
		...
		moss_type = math.random(1,31) -- cliche of more moss at north
...
biome_lib:register_generate_plant({
	...
	max_count = abstract_trunks.Moss_on_trunk_Max_Count,
	rarity = abstract_trunks.Moss_on_trunk_Rarity,
	min_elevation = 1,
	max_elevation = 40,
	plantlife_limit = -0.9,
	check_air = false,
...
-- RooTS 
-----------------------------------------------------------------------------------------------
if abstract_trunks.Roots == true then -- see settings.txt
