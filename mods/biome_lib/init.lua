...
local default_temp = "50,50,{150, 150, 150},112,3,0.5,2.0"
local default_humid = "50,50,{250, 250, 250},9130,3,0.5,2.0"

-- use mapgen noise parameters if possible
-- if not print warning

local temp_table = {}
local temp_string = minetest.setting_get("mg_biome_np_heat")
if temp_string == nil then
	temp_string = default_temp
	print("[Plantlife] WARNING: mg_biome_np_heat not set; using defaults!")
end
...
if humid_string == nil then
	humid_string = default_humid
	print("[Plantlife] WARNING: mg_biome_np_humidity not set; using defaults!")
end
humid_string:gsub("%d+%.?%d*", function(c) table.insert(humid_table,c) end)

local temperature_seeddiff = temp_table[6]
local temperature_octaves = temp_table[7]
local temperature_persistence = temp_table[8]
local temperature_scale = temp_table[3]

local humidity_seeddiff = humid_table[6]
local humidity_octaves = humid_table[7]
local humidity_persistence = humid_table[8]
local humidity_scale = humid_table[3]
...
function biome_lib:populate_surfaces(biome, nodes_or_function_or_model, snodes, checkair)
	...
	for i = 1, #snodes do
		...
		local noise2 = biome_lib.perlin_temperature:get2d({x=pos.x, y=pos.z}) * -1
		local noise3 = biome_lib.perlin_humidity:get2d({x=pos.x+150, y=pos.z+50}) * -1
...
function biome_lib:spawn_on_surfaces(sd,sp,sr,sc,ss,sa)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local noise2 = biome_lib.perlin_temperature:get2d({x=p_top.x, y=p_top.z}) * -1
			local noise3 = biome_lib.perlin_humidity:get2d({x=p_top.x+150, y=p_top.z+50}) * -1
