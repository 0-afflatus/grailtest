...
minetest.register_chatcommand("weather", {
	...
	func = function(playername)
		...
		local mg_heat = string.format("%.2f", minetest.get_heat(ppos))
		local humidity = math.floor(climate_api.environment.get_humidity(ppos) * 100) / 100
		local mg_humidity = string.format("%.2f", minetest.get_humidity(ppos))
		local msg = ""
		if #weathers > 0 then
		...
		msg = msg .. S("It is a @1 @2 right now and humidity is at @3%. @4 | @5", heat_desc, parse_heat(heat), humidity, mg_heat, mg_humidity) .. "\n"
