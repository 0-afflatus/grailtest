...
if not minetest.get_modpath("element")
or element.node_sound_snow_defaults == nil
...
local destruction_handler = function(pos)
	pos.y = pos.y - 1
	if minetest.get_node(pos).name == "element:dirt_with_snow" then
		minetest.set_node(pos, {name = "element:dirt_with_grass"})
	end
end
...
for i = 1,5 do
	...
	minetest.register_node(BLOCK_PREFIX .. i, {
		...
		sounds = element.node_sound_snow_defaults(),
		drop = "element:snow " .. math.ceil(i / 2),
		on_construct = function(pos)
			pos.y = pos.y - 1
			if minetest.get_node(pos).name == "element:dirt_with_grass" then
