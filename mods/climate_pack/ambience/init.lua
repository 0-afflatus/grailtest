...
--= START Ambiance
	...
	local ps, cn = minetest.find_nodes_in_area(
		...
		{x = pos.x + radius, y = pos.y + radius, z = pos.z + radius},
		{
			...
			"element:lava_flowing", "element:lava_source", "element:jungletree",
			"element:water_flowing", "element:water_source",
			"element:river_water_flowing", "element:river_water_source",
			"element:desert_sand", "element:desert_stone", "element:snowblock", "element:permafrost"
		})

	local num_fire = (cn["fire:basic_flame"] or 0) + (cn["fire:permanent_flame"] or 0)
	local num_lava = (cn["element:lava_flowing"] or 0) + (cn["element:lava_source"] or 0)
	local num_water_flowing = (cn["element:water_flowing"] or 0)
	local num_water_source = (cn["element:water_source"] or 0)
	local num_desert = (cn["element:desert_sand"] or 0) + (cn["element:desert_stone"] or 0)
	local num_snow = (cn["element:snowblock"] or 0)
	local num_jungletree = (cn["element:jungletree"] or 0)
	local num_river = (cn["element:river_water_source"] or 0) + (cn["element:river_water_flowing"] or 0)
	local num_frost = (cn["element:permafrost"] or 0)
--[[
print (
	"fr:" .. num_fire,
	"lv:" .. num_lava,
	...
	and tod < 0.8 then
		...
		if num_frost > 150 then
			return {frosty_day = frosty_day}
		end
		...
		if num_frost > 150 then
			return {frosty_night = frosty_night}
		end
...
local still_playing = function(still_playing, player_name)
	...
	if not still_playing.frosty_night then stop_sound(frosty_night, player_name) end
	if not still_playing.frosty_day then stop_sound(frosty_day, player_name) end
