...
function itemshelf.register_shelf(name, def)
	...
	minetest.register_node("itemshelf:"..name, {
		...
		mesh = def.mesh,
		groups = {snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, itemshelf = 1, itemshelf_shown_items = def.shown_items or 4},
		...
		-- Screwdriver support
		--{name = node.name, param1 = node.param1, param2 = node.param2}, user, mode, new_param2)
