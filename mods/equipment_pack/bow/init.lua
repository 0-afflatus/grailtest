--Basic Bow Mod [bow] rev002
--All Code is from Stu's Shooter mod (crossbow)
--License Source Code: 2013 Stuart Jones - LGPL v2.1

bow = {}

minetest.register_alias("bow", "bow:wood_bow")
minetest.register_alias("arrow", "bow:arrow")
...
BASIC_BOW_EXPLOSION_TEXTURE = "bow_hit.png"
...
function bow:spawn_particles(pos, texture)
...
--[[
function --bow:play_node_sound(node, pos)
...
function bow:punch_node(pos, def)
	...
	if item.groups then
		for k, v in pairs(def.groups) do
			...
			if level >= v then
				...
				--bow:play_node_sound(node, pos)
				if item.tiles then
					if item.tiles[1] then
						bow:spawn_particles(pos, item.tiles[1])
					end
				end
				break
			end
...
function bow:is_valid_object(object)
	if object then
...
function bow:get_intersect_pos(ray, plane, collisionbox)
...
function bow:process_round(round)
	local target = {object=nil, distance=10000}
	local p1 = round.pos
	local v1 = round.ray
	for _,ref in ipairs(bow.objects) do
		...
		if p1 and p2 and ref.name ~= round.name then
			...
			if d < round.def.step and d < target.distance then
				...
				local pos = bow:get_intersect_pos(ray, plane, ref.collisionbox)
				if pos then
	...
	if target.object and target.pos then
		...
		if success then
			...
			if user then
				...
				bow:spawn_particles(target.pos, BASIC_BOW_EXPLOSION_TEXTURE)
			end
			return 1
		elseif pos and BASIC_BOW_ALLOW_NODES == true then
			bow:punch_node(pos, round.def)
			return 1
		end
...
local function punch_object(puncher, object)
	if puncher and bow:is_valid_object(object) then
		if puncher ~= object then
			local dir = puncher:get_look_dir()
			local p1 = puncher:getpos()
			local p2 = object:getpos()
			local tpos = get_target_pos(p1, p2, dir, 0)
			bow:spawn_particles(tpos, BASIC_BOW_EXPLOSION_TEXTURE)
...
minetest.register_craftitem("bow:arrow", {
	description = "Arrow",
	inventory_image = "bow_arrow_inv.png",
})

minetest.register_entity("bow:arrow_entity", {
	physical = false,
	visual = "mesh",
mesh = "bow_arrow.b3d",
	...
	on_step = function(self, dtime)
		...
		if self.timer > 0.2 then
			...
			for _,obj in ipairs(objects) do
				if bow:is_valid_object(obj) then
					...
					local ipos = bow:get_intersect_pos(ray, plane, collisionbox)
					if ipos then
			...
			if npos then
				...
				minetest.sound_play("bow_arrow", {pos = self.lastpos, gain = 0.08, max_hear_distance = 2})
			end
			self.timer = 0
		end
	end,
...
minetest.register_tool("bow:wood_bow", {
	...
	wield_image = "bow_loaded.png",


on_use = function(itemstack, user, pointed_thing)
		local inv = user:get_inventory()
if user:get_inventory():get_stack("main", user:get_wield_index()+1): get_name()=="bow:arrow" then
if not minetest.settings:get_bool("creative_mode") then
user:get_inventory():remove_item("main", "bow:arrow 1")
end
		if not minetest.settings:get_bool("creative_mode") then
			itemstack:add_wear(65535/BASIC_BOW_USES)
		end
		itemstack = "bow:wood_bow 1 "..itemstack:get_wear()
		local pos = user:getpos()
		local dir = user:get_look_dir()
		local yaw = user:get_look_yaw()
		if pos and dir and yaw then
			pos.y = pos.y + 1.5
			local obj = minetest.add_entity(pos, "bow:arrow_entity")
			local ent = nil
			if obj then
				ent = obj:get_luaentity()
			end
			if ent then
				minetest.sound_play("bow_throw", {object=obj, gain = 0.08})
				local frame = get_animation_frame(dir)
				...
				if pointed_thing.type ~= "nothing" then
					...
					elseif pointed_thing.type == "node" then
						...
						minetest.after(0.2, function(object, pos, npos)
							...
							--bow:play_node_sound(node)
							minetest.sound_play("bow_arrow", {gain = 0.08, max_hear_distance = 2, pos = npos})
						end, obj, tpos, ppos)
						return itemstack
					end
				end
...
dungeon_loot.register({
	{name = "bow:wood_bow", chance = 0.1, y = {-1023, 512}},
	{name = "bow:arrow", chance = 0.2, count = {1, 8}, y = {-1023, 512}},
})
