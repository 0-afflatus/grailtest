...
local function node_ok(pos)
	...
	if not node then
		return minetest.registered_nodes["element:dirt"]
	end
...
local destroy_item = tonumber(minetest.settings:get("destroy_item")) or 1
...
core.register_entity(":__builtin:item", {
	...
	on_step = function(self, dtime)
		...
		if minetest.registered_nodes[nod.name].liquidtype == "flowing" then
			...
			local vec = quick_flow(self.object:get_pos(),
				node_ok(self.object:get_pos()))
