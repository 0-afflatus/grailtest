...
abstract_ferns.grow_giant_tree_fern = function(pos)
	...
	if minetest.get_node(pos_01).name ~= "air"
			...
			and minetest.get_node(pos_01).name ~= "plant:junglegrass" then
		return
	end
	...
	--[[ all the checking for air below is to prevent some ugly
	bugs (incomplete trunks of neighbouring trees), it's a bit
	slower, but worth the result]]
